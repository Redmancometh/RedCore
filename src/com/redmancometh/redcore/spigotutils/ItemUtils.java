...
public class ItemUtils
{
    ...
    {
        if (in == null) return "0:-1 0";
        StringBuilder out = new StringBuilder();
        out.append(in.getType().name());
        if (in.getDurability() != 0) out.append(':').append(in.getDurability());
        if (in.getAmount() != 1) out.append(' ').append(in.getAmount());
        ItemMeta meta = in.getItemMeta();
        if (meta == null) return out.toString();
        if (ver.isAbove(v1_8)) for (ItemFlag f : meta.getItemFlags())
            out.append(" hide:").append(f.name().substring(5));
        if (meta.hasDisplayName()) out.append(" name:").append(escapeText(meta.getDisplayName()));
        if (meta.hasLore()) out.append(" lore:").append(escapeText(StringUtils.join(meta.getLore(), '\n')));
        ...
        if (meta instanceof BookMeta)
        {
            ...
            if (bmeta.hasAuthor()) out.append(" author:").append(bmeta.getAuthor());
            if (bmeta.hasTitle()) out.append(" title:").append(bmeta.getTitle());
            for (String page : bmeta.getPages())
                out.append(" page:").append(escapeText(page));
        }
        if (ver.isAbove(v1_8)) if (meta instanceof BannerMeta)
        {
        ...
        } else if (meta instanceof PotionMeta)
        {
            ...
            for (PotionEffect e : bmeta.getCustomEffects())
            {
                ...
                if (ver.isAbove(v1_8)) if (!e.hasParticles()) out.append(":np");
                if (!e.isAmbient()) out.append(":na");
            }
        } else if (meta instanceof SkullMeta)
        {
            SkullMeta bmeta = (SkullMeta) meta;
            if (bmeta.hasOwner()) out.append(" owner:").append(bmeta.getOwner());
    ...
    public static ItemStack addLoreStorageMeta(ItemStack is, Object... values)
    {
        ...
        for (Object o : values)
        {
            ...
            if (id == -1) lore.add(prefix + o + suffix);
            else lore.set(id, prefix + o + suffix);
            key = true;
        }
        meta.setLore(lore);
        is.setItemMeta(meta);
    ...
    public static boolean containsItem(Iterable<ItemStack> source, ItemStack is)
    {
        ...
        {
            if (itemSimilar(i, is)) return true;
        }
        return false;
    }
    ...
    public static int countItemSpace(Inventory inv, ItemStack is, int maxStack)
    {
        ...
        {
            ItemStack current = inv.getItem(i);
            if (current == null || current.getType() == Material.AIR) space += maxStack;
            else if (itemSimilar(current, is)) space += maxStack - current.getAmount();
        }
        return space;
    }
    ...
    {
        if (is == null || is.getType() == Material.AIR || !is.hasItemMeta()) return def;
        is = is.clone();
        ItemMeta meta = is.getItemMeta();
        if (!meta.hasLore()) return def;
        ...
        for (int i = st; i < l; i++)
        {
            ...
            try
            {
                ...
                if (enc == null)
                {
                    if (ver.isAbove(v1_8))
                    {
                        if (s[0].equals("HIDE")) meta.addItemFlags(ItemFlag.valueOf("HIDE_" + s[1].toUpperCase()));
                    }
                    if (s[0].equals("NAME"))
                    {
            ...
            for (String[] s : remaining)
            {
                try
                {
                    if (s[0].equals("COLOR"))
                    {
                        ...
                        else bmeta.setColor(Color.fromRGB(Integer.parseInt(color[0], 16)));
                    }
                } catch (Throwable e)
                {
            ...
            for (String[] s : remaining)
            {
                try
                {
                    ...
                    if (type != null)
                    {
                        ...
                        if (ver.isAbove(v1_8))
                        {
                            bmeta.addCustomEffect(new PotionEffect(type, Integer.valueOf(s2[0]), Integer.valueOf(s2[1]), !ArrayUtils.contains(s2, "na"), !ArrayUtils.contains(s2, "np")), false);
                        } else
                        {
                            bmeta.addCustomEffect(new PotionEffect(type, Integer.valueOf(s2[0]), Integer.valueOf(s2[1]), !ArrayUtils.contains(s2, "na")), false);
                        }
                    }
                } catch (Throwable e)
                {
        ...
        } else if (meta instanceof EnchantmentStorageMeta)
        {
            ...
            for (String[] s : remaining)
            {
                try
                {
                    ...
                    if (enc != null) bmeta.addStoredEnchant(enc, Integer.valueOf(s[1]), true);
                } catch (Throwable e)
                {
