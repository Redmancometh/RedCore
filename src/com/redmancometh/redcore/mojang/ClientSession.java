...
import static com.redmancometh.redcore.mojang.WebApi.postWithHeader;
...
public class ClientSession
{
    ...
    public void changeSkin(String newSkinURL, boolean slimModel)
    {
        try
        {
            postWithHeader("https://api.mojang.com/user/profile/" + selectedProfile.id + "/skin", "Authorization", "Bearer " + accessToken, "model=" + (slimModel ? "slim&url=" : "&url=") + URLEncoder.encode(newSkinURL, "UTF-8"));
    ...
    public boolean invalidate()
    {
        try
        {
            post("https://authserver.mojang.com/invalidate", "{\"accessToken\": \"" + accessToken + "\"," + "\"clientToken\": \"" + clientToken + "\"}");
            return true;
        } catch (Throwable e)
        {
            return false;
    ...
    public boolean refresh()
    {
        try
        {
            String s = post("https://authserver.mojang.com/refresh", "{\"accessToken\": \"" + accessToken + "\"," + "\"clientToken\": \"" + clientToken + "\"}");
    ...
    public boolean signOut()
    {
        try
        {
            post("https://authserver.mojang.com/signout", "{\"username\": \"" + username + "\"," + "\"password\": \"" + password + "\"}");
            return true;
        } catch (Throwable e)
        {
            return false;
    ...
    public boolean validate()
    {
        try
        {
            post("https://authserver.mojang.com/validate", "{\"accessToken\": \"" + accessToken + "\"," + "\"clientToken\": \"" + clientToken + "\"}");
            return true;
        } catch (Throwable e)
        {
            return false;
